...
ucebolunenlerintoplami = ucetambolunenler.reduce(function (sum, item) {
  return sum + item;
}, 0);

//3d çözümü

besyuzdenkucuksayilar = sayilar.filter(function (item) {
  return item < 500;
});

//3e çözümü

siralisayilar = besyuzdenkucuksayilar.sort(function (x1, x2) {
  return x1 - x2;
});

//3f çözümü

let cntOfEachNumberObj = sayilar.reduce(function (obj, num) {
  if (obj[num]) {
    obj[num]++;
  } else {
    obj[num] = 1;
...
tekraredensayilar = [];
for (const key in cntOfEachNumberObj) {
  if (cntOfEachNumberObj[key] > 1) {
    tekraredensayilar.push(
      `${key} sayısı ${cntOfEachNumberObj[key]} kere tekrar edilmiştir`
    );
  }
}
